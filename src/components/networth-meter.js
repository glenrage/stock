...
class NetWorthMeter extends Component {
  ...
  render() {
    ...
    let netWorthPercents = netWorthPortions.map(wp => {
      let percent = parseFloat((net_worth_meter * wp) / netWorth.current);
    ...
    return (
      <div className="netWorth-meter">
        <svg
          ...
          <defs>
            <radialGradient id="netWorthGrad" cx="50%" cy="100%" r="70%">
              ...
              <stop stopColor="#2b2f32" offset="0">
                <animate attributeName="offset" dur="0.3s" values="0;.20;.40;.60;.80;.95;" fill="freeze"/>
              </stop>
            </radialGradient>
          </defs>
          {netWorthPercents.map((wp, idx) => {
            ...
            return (
              <path
								...
								key={'stock-' + idx} fill={`#${colors.styleOne[idx]}`}
								d={`M${centerX},${centerY} l${centerX},0 A${centerX},${centerY} 0 0,0 ${p.x},${p.y} z`}
							>
                <animateTransform
								attributeName="transform"
								type="rotate"
								from={`0 ${centerX} ${centerY}`} to={`${ - angles[idx]} ${centerX} ${centerY}`}
								dur="0.5s"
								fill="freeze"/>
              </path>
            )
          ...
          <text className="netWorth-label"
						x={centerX}
						y={centerY - 75}
						textAnchor="middle">
            PORTFOLIO VALUE
          </text>
          <text className="netWorth-current"
						x={centerX}
						y={centerY - 40}
						textAnchor="middle"
					>
